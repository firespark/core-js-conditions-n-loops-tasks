...
function convertToRomanNumerals(num) {
  const tens = ['', 'X', 'XX', 'XXX'];
  const items = ['', 'I', 'II', 'III', 'IV', 'V', 'VI', 'VII', 'VIII', 'IX'];

  return tens[Math.floor(num / 10)] + items[num % 10];
}
...
function isContainNumber(num, digit) {
  ...
  const strChecked = String(digit);
  for (let i = 0; i < str.length; i += 1) {
    if (str[i] === strChecked) return true;
  }
  return false;
}
...
function sortByAsc(arr, low = 0, high = arr.length - 1) {
  ...
  if (low < high) {
    ...
    sortByAsc(newArr, low, pi - 1);
    sortByAsc(newArr, pi + 1, high);
  }
  return newArr;
}
